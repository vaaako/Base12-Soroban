...
function AbacusCtrl(type) {
	this.type = type; // 0 Japanese, 1 Chinese, 2 Vako
...
function Abacus(parentDivId, type) {
	...
	this.init = function() {
		...
		canvas.height= 60 + (abacusCtrl.beadPerLine+2) * abacusCtrl.beadHeight + EXTRA_SPACE_SIZE;
	...
	this.update = function() {
		...
		for(var i=0; i < abacusCtrl.beadLines; i++) {
			...
			for(var j=0; j < abacusCtrl.beadPerLine; j++) {
				...
				if(abacusCtrl.nodes[n].active) {
					...
					// Calc total
					valueTotal += abacusCtrl.nodes[n].value * (12**i);
				}
			}
		...
		var textX = -140 + abacusCtrl.beadLines * abacusCtrl.beadSpacing - 6 * abacusCtrl.beadSpacing;
		var textY = 100 + (abacusCtrl.beadPerLine + 2) * abacusCtrl.beadHeight;
